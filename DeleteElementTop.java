...
//DELETE ELEMENT TOP
public class DeleteElementTop {
	//Function to delete an element
	static int deleteElement(String name[],int grade[], int n)
	{
		//deleting element on TOP
			for (int i = 0; i < n - 1; i++)
			{
				name[i] = name [i + 1];
				grade[i] = grade[i + 1];
...
public static Scanner input = new Scanner(System.in);
	...
	public static void main(String[] args)throws Exception {
		...
		//Display data on the array SORTED
				...
				//Delete TOP implementation
				
				System.out.println("\n// ==DELETE IMPLEMENTATION== ");
				System.out.println("\nPRESS ENTER TO DELETE THE TOP ELEMENT ");
				input.nextLine(); //where: key variable is the name search

				int n=name.length;		//the n is the size of the array
				n = deleteElement(name, grade, n);
